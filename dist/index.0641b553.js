...
function getCurrency(country) {
    if (country.currencies.length === 1) // 1 valuta: and you can pay with [currency]'s
    return `and you can pay with ${country.currencies[0].name}'s`;
    else // 2 valuta's: and you can pay with [currency]'s and [currency]'s
    return `and you can pay with ${country.currencies[0].name}'s and ${country.currencies[1].name}'s`;
}
function getLanguage(language) {
    if (language.languages.length === 1) // 1 taal: They speak [language]
    return `They speak ${language.languages[0].name}`;
    else if (language.languages.length === 2) // 2 talen: They speak [language] and [language]
    return `They speak ${language.languages[0].name} and ${language.languages[1].name}`;
    else if (language.languages.length === 3) // 3 talen: They speak [language], [language] and [language]
    return `They speak ${language.languages[0].name}, ${language.languages[1].name} and ${language.languages[2].name}`;
    else return `They speak ${language.languages[0].name}, ${language.languages[1].name} and ${language.languages[2].name} and more`;
}
async function searchRequest(request) {
    try {
        const countries = await (0, _axiosDefault.default).get("https://restcountries.com/v2/all");
...
submitForm.addEventListener("submit", (event)=>{
    ...
    searchRequest(inputField.value);
    inputField.value = "";
});
...
var Buffer = require("82ef0c7b4435b4d9").Buffer;
"use strict";
/**
...
},{"82ef0c7b4435b4d9":"fCgem","../utils.js":"5By4s","../core/AxiosError.js":"3u8Tl","../platform/node/classes/FormData.js":"aFlee","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"fCgem":[function(require,module,exports) {
/*!
...
var base64 = require("48372fc94e6f00be");
var ieee754 = require("43572fb425301c36");
...
},{"48372fc94e6f00be":"eIiSV","43572fb425301c36":"cO95r"}],"eIiSV":[function(require,module,exports) {
"use strict";
